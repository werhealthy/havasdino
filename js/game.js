import { GAME_HEIGHT, GAME_WIDTH, GROUND_Y, SPRITES } from "./config.js";
import { Player } from "./player.js";
import { pickObstacle } from "./obstacle.js";
...
export class Game {
  ...
  startRun(charKey) {
    ...
    this.player = new Player(charKey, this.images[charKey]);
    this.obstacles = [];
    this.score = 0;
    this.speed = 155;
    this.spawnTimer = 0;
  ...
  setDuck(on) {
    this.input.duck = on;
    if (this.state === "playing") this.player?.setDuck(on);
  }
  ...
  update(dt) {
    ...
    if (this.spawnTimer > this.nextSpawn) {
      ...
      if (clear) {
        this.obstacles.push(pickObstacle(this.score));
        this.spawnTimer = 0;
  ...
  drawBackground() {
    ...
    for (let i = -1; i < GAME_WIDTH / 16 + 2; i++) {
      const x = i * 16 - this.groundOffset;
  ...
  drawHud() {
    ...
    if (this.player) {
      ...
      ctx.drawImage(img, frame.x, frame.y, frame.w, frame.h, 8, 8, frame.w, frame.h);
      ctx.globalAlpha = 1;
      ctx.textAlign = "left";
      ctx.fillText(`Watching: ${SPRITES[other].label}`, 36, 18);
    }
  }
  ...
  drawMenu() {
    ...
    ctx.drawImage(this.images.boss, boss.x, boss.y, boss.w, boss.h, 132, 50, boss.w, boss.h);
    ctx.drawImage(this.images.francesco, fr.x, fr.y, fr.w, fr.h, 220, 50, fr.w, fr.h);
  ...
  render() {
    ...
    for (const obstacle of this.obstacles) obstacle.draw(this.ctx);
