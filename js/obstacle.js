...
(function () {
  const { GAME_WIDTH, GROUND_Y, OBSTACLE_TYPES } = App.CONFIG;
  ...
  function pickObstacle(score) {
    ...
    Object.entries(OBSTACLE_TYPES).forEach(([key, type]) => {
      if (score >= type.minScore) {
        if (type.rare && Math.random() > type.rare) return;
        options.push({ ...type, key });
      }
    });

    const choice = options[Math.floor(Math.random() * options.length)] || { ...OBSTACLE_TYPES.invoice, key: "invoice" };
    return new Obstacle(choice);
  }

  App.Obstacle = Obstacle;
  App.pickObstacle = pickObstacle;
})();
